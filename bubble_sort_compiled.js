...
(function () {
  ...
  var x = d3.scale.ordinal().domain(d3.range(NUM_ELEMENTS + 1)).rangePoints([MARGIN, SVG_WIDTH - MARGIN]);
  ...
  var groups = svg.selectAll("g.elements").data(array).enter().append("g").attr('class', 'element').attr("transform", groupTransform);
  ...
  var checks = groups.append('text').attr('transform', checkTransform).attr('class', 'checks');
  ...
  function doNext(shouldContinue) {
    ...
    groups.data(newState.array, Number).transition().duration(STEP_DURATION).attr("transform", groupTransform).each(function (d, i) {
      if (newState.done.indexOf(i) !== -1) {
        d3.select(this.querySelector('text.checks')).text('✔');
      }
    });

    arrows.data(newState.cursors).exit().remove();

    arrows.transition().duration(STEP_DURATION).attr("transform", cursorTransform);
